...
- one **free Postgres** database, and
- one **free web service** that builds both `server/` and `web/`, copies the
  built frontend into `server/web-dist` (served as static files by Express,
  so there's a single deployed service and no CORS setup), runs
  `prisma migrate deploy` + the seed script, and starts the API.

To deploy:

1. Push this repo to GitHub (ask first if using an assistant — pushing is a
   publishing action).
2. In the Render dashboard: **New → Blueprint**, connect the repo, and
   Render will read `render.yaml` and provision both services automatically.
3. `JWT_SECRET` is auto-generated by the blueprint. `SMTP_*`/`TWILIO_*` are
   optional — set them in the Render dashboard's Environment tab if you
   want real email/SMS sends; otherwise notifications are just logged (see
   [Notifications](#notifications)).
4. First boot runs migrations + seed automatically, so the same seeded
   accounts from local dev work immediately on the hosted URL.

Free-tier note: the web service spins down after inactivity and takes ~30s
to wake on the next request — expected on Render's free plan, not a bug.
